...
int main(int argc, char *argv[])
{
  ...
  const int Nx = getenv_var<int>(VAR_NX, 500);
  const int Ny = getenv_var<int>(VAR_NY, 500);
  ...
  const double ymax = getenv_var<double>(VAR_YMAX, 0.05);
  ...
  const Laplacian2D::CL CL_bas = getenv_var<Laplacian2D::CL>(VAR_CL_BAS, Laplacian2D::CL::NEUMANN);
  const Laplacian2D::CL CL_haut = getenv_var<Laplacian2D::CL>(VAR_CL_HAUT, Laplacian2D::CL::NEUMANN);
  const Laplacian2D::CL CL_gauche = getenv_var<Laplacian2D::CL>(VAR_CL_GAUCHE, Laplacian2D::CL::NEUMANN_NON_CONSTANT); //Peut valoir "Neumann_non_constant", cette condition est celle proposée par Mme Baranger dans notre TER, un flux au bord affine par morceaux
  const Laplacian2D::CL CL_droite = getenv_var<Laplacian2D::CL>(VAR_CL_DROITE, Laplacian2D::CL::NEUMANN);
  ...
  //EC_ClassiqueP Lap;
//
  //Lap.Initialize(xmin, xmax, ymin, ymax, Nx, Ny, a, deltaT, Me, Np, Source, save_all_file, save_points_file, saved_points);
  //Lap.InitializeCI(CI);
